...
public class ChatRowBigExpression extends ChatRowText {
    ...
    protected void onInflatView() {
        inflater.inflate(message.direct() == Message.Direct.RECEIVE ?
                com.hyphenate.helpdesk.R.layout.hd_row_received_bigexpression : com.hyphenate.helpdesk.R.layout.hd_row_sent_bigexpression, this);
    }
    ...
    public void onSetUpView() {
        ...
        if(emojicon != null){
            if(emojicon.getBigIcon() != 0){
                Glide.with(activity).load(emojicon.getBigIcon()).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_expression)).into(imageView);
//                Glide.with(activity).load(emojicon.getBigIcon()).placeholder(R.drawable.hd_default_expression).into(imageView);
            }else if(emojicon.getBigIconPath() != null){
                Glide.with(activity).load(emojicon.getBigIconPath()).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_expression)).into(imageView);
//                Glide.with(activity).load(emojicon.getBigIconPath()).placeholder(R.drawable.hd_default_expression).into(imageView);
            }else{
                imageView.setImageResource(com.hyphenate.helpdesk.R.drawable.hd_default_expression);
            }
        }
