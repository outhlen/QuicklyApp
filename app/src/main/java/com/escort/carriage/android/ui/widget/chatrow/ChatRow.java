...
public abstract class ChatRow extends LinearLayout {
    ...
    protected static final String TAG = com.hyphenate.helpdesk.easeui.widget.chatrow.ChatRow.class.getSimpleName();
...
//    protected TextView deliveredView;
    ...
    private void initView() {
        onInflatView();
        timeStampView = (TextView) findViewById(com.hyphenate.helpdesk.R.id.timestamp);
        userAvatarView = (ImageView) findViewById(com.hyphenate.helpdesk.R.id.iv_userhead);
        bubbleLayout = findViewById(com.hyphenate.helpdesk.R.id.bubble);
        usernickView = (TextView) findViewById(com.hyphenate.helpdesk.R.id.tv_userid);

        progressBar = (ProgressBar) findViewById(com.hyphenate.helpdesk.R.id.progress_bar);
        statusView = (ImageView) findViewById(com.hyphenate.helpdesk.R.id.msg_status);
...
//        deliveredView = (TextView) findViewById(R.id.tv_delivered);
    ...
    private void setUpBaseView() {
        // 设置用户昵称头像，bubble背景等
        TextView timestamp = (TextView) findViewById(com.hyphenate.helpdesk.R.id.timestamp);
        if (timestamp != null) {
            if (position == 0) {
            ...
            public void run() {
                if (message.status() == Message.Status.FAIL) {
                    if (message.error() == Error.MESSAGE_INCLUDE_ILLEGAL_CONTENT) {
                        ToastHelper.show(activity, com.hyphenate.helpdesk.R.string.send_fail);
                    } else {
                        ToastHelper.show(activity, activity.getString(com.hyphenate.helpdesk.R.string.send_fail) + activity.getString(com.hyphenate.helpdesk.R.string.connect_failuer_toast));
                    }
                }
