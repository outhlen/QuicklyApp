...
public class ChatRowVoice extends ChatRowFile {
    ...
    protected void onInflatView() {
        inflater.inflate(message.direct() == Message.Direct.RECEIVE ?
                com.hyphenate.helpdesk.R.layout.hd_row_received_voice : com.hyphenate.helpdesk.R.layout.hd_row_sent_voice, this);
    }

    @Override
    protected void onFindViewById() {
        voiceLengthView = (TextView) findViewById(com.hyphenate.helpdesk.R.id.tv_length);
        readStatusView = (ImageView) findViewById(com.hyphenate.helpdesk.R.id.iv_unread_voice);
    }
    ...
    protected void onBubbleClick() {
        ...
        if (message.direct() == Message.Direct.SEND){
            ...
            if (file.exists()){
                ...
                ToastHelper.show(activity, com.hyphenate.helpdesk.R.string.is_down_please_wait);
            }else{
    ...
    private void playVoice(View v, String localPath, final boolean isSend){
        ...
        if (((MessageAdapter)adapter).animView != null){
            ...
            if (preIsSend){
                ((MessageAdapter)adapter).animView.setBackgroundResource(com.hyphenate.helpdesk.R.drawable.hd_chatto_voice_playing);
            }else{
                ((MessageAdapter)adapter).animView.setBackgroundResource(com.hyphenate.helpdesk.R.drawable.hd_chatfrom_voice_playing);
            }
        ...
        ((MessageAdapter)adapter).animView = v.findViewById(com.hyphenate.helpdesk.R.id.id_recorder_anim);
        ((MessageAdapter)adapter).animView.setTag(isSend);
        if (isSend){
            ((MessageAdapter)adapter).animView.setBackgroundResource(com.hyphenate.helpdesk.R.drawable.hd_voice_to_icon);
        }else{
            ((MessageAdapter)adapter).animView.setBackgroundResource(com.hyphenate.helpdesk.R.drawable.hd_voice_from_icon);
            if (!message.isListened()){
                readStatusView.setVisibility(View.GONE);
