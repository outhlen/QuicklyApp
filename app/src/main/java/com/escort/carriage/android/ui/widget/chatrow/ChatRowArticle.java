...
public class ChatRowArticle extends ChatRow {
	...
	protected void onInflatView() {
		inflater.inflate(com.hyphenate.helpdesk.R.layout.hd_row_received_articles, this);
	}

	@Override
	protected void onFindViewById() {
		artticlesContainer = (LinearLayout) findViewById(com.hyphenate.helpdesk.R.id.articlesContainer);
	...
	private void addViews(ArticlesInfo msgArticles) {
		...
		if (msgArticles.getArticles().size() == 1) {
			...
			View view = inflater.inflate(com.hyphenate.helpdesk.R.layout.hd_row_article_single_main, null);
			...
			if (bean.getTitle() != null) {
				((TextView) view.findViewById(com.hyphenate.helpdesk.R.id.article_main_title)).setText(bean.getTitle());
			}

			((TextView) view.findViewById(com.hyphenate.helpdesk.R.id.article_create_time)).setText(bean.getDate());
...
//			Glide.with(getContext()).load(bean.getPicurl()).error(R.drawable.hd_img_missing).into((ImageView) view.findViewById(R.id.article_main_pic));
			Glide.with(getContext()).load(bean.getPicurl()).apply(RequestOptions.errorOf(com.hyphenate.helpdesk.R.drawable.hd_img_missing)).into((ImageView) view.findViewById(com.hyphenate.helpdesk.R.id.article_main_pic));

			if (bean.getDescription() != null) {
				((TextView) view.findViewById(com.hyphenate.helpdesk.R.id.article_main_digit)).setText(bean.getDescription());
			}

			(view.findViewById(com.hyphenate.helpdesk.R.id.ll_article_detail)).setOnClickListener(new OnClickListener() {
				@Override
			...
			for (int i = 1; i < msgArticles.getArticles().size(); i++) {
				...
				divide.setBackgroundColor(context.getResources().getColor(com.hyphenate.helpdesk.R.color.articles_divider_color));
	...
	private View createArticles(final ArticlesInfo.ArticleItem bean, Boolean isFirst) {
		...
		if (isFirst) {
			view = (RelativeLayout)inflater.inflate(com.hyphenate.helpdesk.R.layout.hd_row_article_main, null);
			RelativeLayout mainLayout = (RelativeLayout) view.findViewById(com.hyphenate.helpdesk.R.id.rl_main);
			ImageView mainImage = (ImageView) view.findViewById(com.hyphenate.helpdesk.R.id.iv_main);
			TextView mainText = (TextView) view.findViewById(com.hyphenate.helpdesk.R.id.tv_main);
			LinearLayout mainTextLayout = (LinearLayout) view.findViewById(com.hyphenate.helpdesk.R.id.ll_main_text);
...
//			Glide.with(getContext()).load(bean.getPicurl()).error(R.drawable.hd_img_missing).into(mainImage);
			Glide.with(getContext()).load(bean.getPicurl()).apply(RequestOptions.errorOf(com.hyphenate.helpdesk.R.drawable.hd_img_missing)).into(mainImage);
...
//			Glide.with(getContext()).load(bean.getPicurl()).error(R.drawable.hd_img_missing).into(imageView);
			Glide.with(getContext()).load(bean.getPicurl()).apply(RequestOptions.errorOf(com.hyphenate.helpdesk.R.drawable.hd_img_missing)).into(imageView);
			view.addView(imageView, ivLp);

			TextView textView = new TextView(context);
			textView.setPadding(0, 0, DensityUtil.dip2px(context, 10), 0);
			textView.setTextColor(context.getResources().getColor(com.hyphenate.helpdesk.R.color.article_row_text_color));
