...
public class ChatRowCustomEmoji extends ChatRow {
	...
	protected void onInflatView() {
		inflater.inflate(message.direct() == Message.Direct.RECEIVE ? com.hyphenate.helpdesk.R.layout.hd_row_received_custom_emoji : com.hyphenate.helpdesk.R.layout.hd_row_sent_custom_emoji, this);
	}

	@Override
	protected void onFindViewById() {
		percentageView = (TextView) findViewById(com.hyphenate.helpdesk.R.id.percentage);
		imageView = (ImageView) findViewById(com.hyphenate.helpdesk.R.id.image);
	...
	protected void onSetUpView() {
		...
		if (!TextUtils.isEmpty(emojiconEntity.origin.remoteUrl)) {
			...
			if (localOrigin.exists()) {
				Glide.with(getContext()).load(emojiconEntity.origin.localUrl).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_image)).into(imageView);
//				Glide.with(getContext()).load(emojiconEntity.origin.localUrl).placeholder(R.drawable.hd_default_image).into(imageView);
			} else {
				Glide.with(getContext()).load(emojiconEntity.origin.remoteUrl).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_image)).into(imageView);
//				Glide.with(getContext()).load(emojiconEntity.origin.remoteUrl).placeholder(R.drawable.hd_default_image).into(imageView);
			...
			if (localThumb.exists()) {
				Glide.with(getContext()).load(emojiconEntity.thumbnail.localUrl).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_image)).into(imageView);
//				Glide.with(getContext()).load(emojiconEntity.thumbnail.localUrl).placeholder(R.drawable.hd_default_image).into(imageView);
			} else {
				Glide.with(getContext()).load(emojiconEntity.thumbnail.remoteUrl).apply(RequestOptions.placeholderOf(com.hyphenate.helpdesk.R.drawable.hd_default_image)).into(imageView);
