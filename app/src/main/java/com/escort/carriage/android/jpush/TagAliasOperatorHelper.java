...
public class TagAliasOperatorHelper {
            ...
            switch (msg.what){
                case DELAY_SEND_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  TagAliasBean){
                        LogUtils.showI(TAG,"on delay time");
                        sequence++;
                        ...
                        if(context!=null) {
                            ...
                            LogUtils.showE(TAG,"#unexcepted - context was null");
                        }
                    }else{
                        LogUtils.showW(TAG,"#unexcepted - msg obj was incorrect");
                    }
                    break;
                case DELAY_SET_MOBILE_NUMBER_ACTION:
                    if(msg.obj !=null && msg.obj instanceof  String) {
                        LogUtils.showI(TAG, "retry set mobile number");
                        sequence++;
    ...
    public void handleAction(Context context,int sequence,String mobileNumber){
        ...
        LogUtils.showD(TAG,"sequence:"+sequence+",mobileNumber:"+mobileNumber);
    ...
    public void handleAction(Context context,int sequence, TagAliasBean tagAliasBean){
        ...
        if(tagAliasBean == null){
            LogUtils.showW(TAG,"tagAliasBean was null");
            return;
        }
        ...
        if(tagAliasBean.isAliasAction){
            switch (tagAliasBean.action){
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    LogUtils.showW(TAG,"unsupport alias action type");
                    return;
            }
        }else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    LogUtils.showW(TAG,"unsupport tag action type");
                    return;
            }
        }
    }
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        if(!ExampleUtil.isConnected(context)){
            LogUtils.showW(TAG,"no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if(errorCode == 6002 || errorCode == 6014){
            LogUtils.showD(TAG,"need retry");
            if(tagAliasBean!=null){
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        LogUtils.showI(TAG,"action - onTagOperatorResult, sequence:"+sequence+",tags:"+jPushMessage.getTags());
        LogUtils.showI(TAG,"tags size:"+jPushMessage.getTags().size());
        init(context);
...
//            ExampleUtil.showToast("获取缓存记录失败", context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtils.showI(TAG,"action - modify tag Success,sequence:"+sequence);
            setActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action)+" tags success";
            LogUtils.showI(TAG,logs);
            ExampleUtil.showToast(logs, context);
        }else{
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage){
        int sequence = jPushMessage.getSequence();
        LogUtils.showI(TAG,"action - onCheckTagOperatorResult, sequence:"+sequence+",checktag:"+jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
//            ExampleUtil.showToast("获取缓存记录失败", context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtils.showI(TAG,"tagBean:"+tagAliasBean);
            setActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtils.showI(TAG,"action - onAliasOperatorResult, sequence:"+sequence+",alias:"+jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = (TagAliasBean)setActionCache.get(sequence);
        if(tagAliasBean == null){
//            ExampleUtil.showToast("获取缓存记录失败", context);
        ...
        if(jPushMessage.getErrorCode() == 0){
            LogUtils.showI(TAG,"action - modify alias Success,sequence:"+sequence);
            setActionCache.remove(sequence);
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogUtils.showI(TAG,"action - onMobileNumberOperatorResult, sequence:"+sequence+",mobileNumber:"+jPushMessage.getMobileNumber());
        init(context);
        if(jPushMessage.getErrorCode() == 0){
            LogUtils.showI(TAG,"action - set mobile number Success,sequence:"+sequence);
            setActionCache.remove(sequence);
        }else{
